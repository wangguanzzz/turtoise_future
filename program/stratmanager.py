...
class StrategyManager():
    ...
    def __init__(self, symbol):
        self.df = self._extract_data(symbol)
        self.sharpe = 0

    # Extract data
    def _extract_data(self, symbol):
        data = ak.futures_zh_daily_sina(symbol=symbol)
        data.rename(columns={"date": "Date","open":"Open","high": "High", "low": "Low", "close": "Close","volume": "Volume",'hold':'Hold'}, inplace=True)
        data.set_index('Date', inplace=True)
        data = data[["Open", "High", "Low", "Close", "Volume","Hold"]]
